...
func TestBenchmark_Put_OrderKeys(t *testing.T) {
	...
	stdMap := make(map[util.Integer]int)
	elapsed := util.ExecCost(func() {
		for i := 0; i < benchmark; i++ {
			stdMap[util.Integer(i)] = i
		}
	})
...
func TestBenchmark_RandKeys(t *testing.T) {
	...
	elapsed := util.ExecCost(func() {
		for i := 0; i < benchmark; i++ {
			stdMap[util.Integer(rand.Intn(n))] = i
		}
	})
	t.Logf("stdMap.Put cost %v", elapsed)
	elapsed = util.ExecCost(func() {
		for i := 0; i < benchmark; i++ {
			_ = stdMap[util.Integer(rand.Intn(n))]
		}
	})
	t.Logf("stdMap.Get cost %v", elapsed)
	elapsed = util.ExecCost(func() {
		for i := 0; i < benchmark; i++ {
			delete(stdMap, util.Integer(rand.Intn(n)))
		}
	})
	...
	elapsed = util.ExecCost(func() {
		for i := 0; i < benchmark; i++ {
			hm.Put(util.Integer(rand.Intn(n)), i)
		}
	})
	t.Logf("hashmap.Put cost %v", elapsed)
	elapsed = util.ExecCost(func() {
		for i := 0; i < benchmark; i++ {
			hm.Get(util.Integer(rand.Intn(n)))
		}
	})
	t.Logf("hashmap.Get cost %v", elapsed)
	elapsed = util.ExecCost(func() {
		for i := 0; i < benchmark; i++ {
			hm.Del(util.Integer(rand.Intn(n)))
		}
	})
	...
	elapsed = util.ExecCost(func() {
		for i := 0; i < benchmark; i++ {
			avl.Put(util.Integer(rand.Intn(n)), i)
		}
	})
	t.Logf("avl.Put cost %v", elapsed)
	elapsed = util.ExecCost(func() {
		for i := 0; i < benchmark; i++ {
			avl.Get(util.Integer(rand.Intn(n)))
		}
	})
	t.Logf("avl.Get cost %v", elapsed)
	elapsed = util.ExecCost(func() {
		for i := 0; i < benchmark; i++ {
			avl.Del(util.Integer(rand.Intn(n)))
		}
	})
	...
	elapsed = util.ExecCost(func() {
		for i := 0; i < benchmark; i++ {
			bt.Put(util.Integer(rand.Intn(n)), i)
		}
	})
	t.Logf("binarytree.Put cost %v", elapsed)
	elapsed = util.ExecCost(func() {
		for i := 0; i < benchmark; i++ {
			bt.Get(util.Integer(rand.Intn(n)))
		}
	})
	t.Logf("binarytree.Get cost %v", elapsed)
	elapsed = util.ExecCost(func() {
		for i := 0; i < benchmark; i++ {
			bt.Del(util.Integer(rand.Intn(n)))
		}
	})
